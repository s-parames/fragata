...
usage() {
  cat <<'EOF'
Usage:
  bash scripts/onboard_db.sh --input <path.jsonl> --department <aplicaciones|sistemas|bigdata|general|comunicaciones> [options]
...
case "$DEPARTMENT" in
  ...
  general)
    DEPARTMENT="general"
    ;;
  comunicacion|comunicaciones|comunicacions|communication|communications)
    DEPARTMENT="comunicaciones"
    ;;
  *)
    echo "Invalid department: $DEPARTMENT. Use aplicaciones, sistemas, bigdata, general or comunicaciones." >&2
    exit 1
    ;;
esac
