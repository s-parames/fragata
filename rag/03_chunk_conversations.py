...
def build_chunks_for_conversation(row, min_chars, target_chars, max_chars, overlap_turns, min_keep_chars):
    ...
    for m in messages:
        ...
        content_raw = normalize_text(m.get("content_raw") or m.get("content_clean") or "")
        content_retrieval = normalize_text(
            m.get("content_retrieval") or m.get("content_clean") or content_raw
        )
